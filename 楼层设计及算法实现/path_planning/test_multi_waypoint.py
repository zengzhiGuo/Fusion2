...
def test_multi_waypoint_path():
    ...
    logger.info("  途经点1：楼层1 - 消防办公室 (22.1582, 6.4752)")
    logger.info("  途经点2：楼层2 - 厨房 (20.2682, 7.7750)")
    ...
    # 途经点列表
    waypoints = [
        Position(x=22.1582, y=6.4752, map_id='floor1'),  # 途经点1：消防办公室
        Position(x=20.2682, y=7.7750, map_id='floor2')   # 途经点2：厨房
    ]
    ...
    # 5. 执行路径规划
    try:
        result = service.plan_multi_waypoint_path(start_pos, waypoints, end_pos)
...
def test_single_floor_multi_waypoint():
    """测试同楼层多点路径规划"""
    
    logger.info("\n\n" + "=" * 80)
    logger.info("开始同楼层多点路径规划测试")
    logger.info("=" * 80)
    ...
    logger.info("\n步骤4: 执行同楼层多点路径规划...")
    logger.info("-" * 80)
    logger.info("测试案例（楼层1内）：")
    logger.info("  起点：A甲板会议室 (22.4460, 13.4433)")
    logger.info("  途经点1：甲板更衣室 (22.4389, 4.0014)")
    logger.info("  途经点2：消防办公室 (22.1582, 6.4752)")
    logger.info("  终点：轮机更衣室 (18.3756, 14.5180)")
    logger.info("-" * 80)
    ...
    # 途经点列表
    waypoints = [
        Position(x=22.4389, y=4.0014, map_id='floor1'),  # 甲板更衣室
        Position(x=22.1582, y=6.4752, map_id='floor1')   # 消防办公室
    ]
    
    # 终点
    end_pos = Position(x=18.3756, y=14.5180, map_id='floor1')
...
if __name__ == "__main__":
    # 测试1：跨楼层多点路径规划
    success1 = test_multi_waypoint_path()

    # 测试2：同楼层多点路径规划
    success2 = test_single_floor_multi_waypoint()
    
    # 输出总结
    print("\n" + "=" * 80)
    print("测试总结")
    print("=" * 80)
    print(f"跨楼层多点路径规划: {'✓ 通过' if success1 else '✗ 失败'}")
    print(f"同楼层多点路径规划: {'✓ 通过' if success2 else '✗ 失败'}")
    print("=" * 80)
